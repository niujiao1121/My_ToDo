...
**快速修订 TODO（/fix 指令）**：
在已有 TODO Issue 中评论 `/fix` 并附上修改提示，AI 会在保持现有格式的同时智能更新描述、截止日期、父子任务关系等关键字段：
```
/fix 把截止时间调整到下周五，并改为属于 #120，描述补充接口验收标准
```
机器人会自动更新 Issue 标题/正文、父任务标记、里程碑与优先级标签，确保继续满足仓库既有的格式化要求。

对应的 GitHub Actions 工作流已拆分：`ai-create-todo.yml` 负责 `/todo` 创建，`ai-fix-todo.yml` 专注 `/fix` 更新，便于后续独立演进和调试。
